...
function BuddyDetail() {
  ...
  if (!buddy) {
    return (
      <AppShell>
        <div className="p-10">
          <p>Buddy not found.</p>
          <Link to="/matches" className="text-primary font-bold">
            Back to matches
          </Link>
        </div>
      </AppShell>
  ...
  async function request() {
    ...
    console.log("[match] request clicked", {
  ...
  return (
    <AppShell>
      <div className="max-w-4xl mx-auto p-6 md:p-10 space-y-6">
        <Link
          to="/matches"
          className="inline-flex items-center gap-2 text-navy text-sm font-semibold"
        >
        ...
        <FFCard className="p-0 overflow-hidden">
          <div className="px-6 md:px-8 pt-6 pb-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              ...
              <FFButton onClick={request} size="lg">
                Request match <Icon name="favorite" />
              </FFButton>
            </div>
            ...
            <div className="mt-8 grid md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                <h3 className="font-bold mb-2 flex items-center gap-2">
                  <Icon name="auto_stories" className="text-primary" /> About me
                </h3>
              ...
              {sharedInterests.length > 0 && (
                <Block icon="favorite" title="Shared with you" className="md:col-span-2">
                  <div className="flex flex-wrap gap-2">
                    {sharedInterests.map((l) => (
                      <Tag key={l} tone="coral">
                        {l}
                      </Tag>
                    ))}
                  </div>
              ...
              <div className="md:col-span-2 bg-primary-soft/50 rounded-lg p-5">
                <p className="font-bold text-navy text-xs uppercase tracking-widest mb-2">
                  Why this match
                </p>
                <ul className="text-navy/80 space-y-1">
                  {reasons.map((r) => (
                    <li key={r} className="flex items-start gap-2">
                      <Icon name="check_circle" className="text-primary text-sm mt-0.5" />
                      {r}
                    </li>
                  ))}
                </ul>
