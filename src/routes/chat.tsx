...
function Chat() {
  ...
  useEffect(() => {
    ...
    fetchLatestMatchRequest(currentUserId).then((request) => {
      ...
      console.log("[chat] restored cloud match", {
        currentUserId,
        targetProfileId,
        requestId: request.id,
        status: request.status,
  ...
  useEffect(() => {
    ...
    if (match.buddyId.startsWith("acct:")) {
      ...
      fetchProfile(targetProfileId).then((u) => {
        console.log("[chat] profile fetch result", {
          targetProfileId,
          found: Boolean(u),
          status: match.status,
        });
      ...
      setMessages([
        {
          id: "1",
          from: "buddy",
          text: `Hey! So happy we matched 🎉 I'm ${buddy.name.split(" ")[0]}, ask me anything.`,
          at: "now",
        },
      ]);
    }
        ...
        <div className="max-w-xl mx-auto p-6 md:p-10">
          <FFCard className="text-center py-12 space-y-4">
            ...
            <p className="text-muted-foreground">
              Once a senior buddy accepts your request, you'll be able to chat here.
            </p>
            <Link to={match ? "/match-status" : "/matches"}>
              <FFButton>{match ? "Check status" : "Find a buddy"}</FFButton>
            </Link>
          </FFCard>
        </div>
      </AppShell>
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-80px)] md:h-screen flex flex-col">
        ...
        <FFCard className="flex items-center gap-3 p-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {buddy.helpTags.slice(0, 3).map((t) => (
          ...
          <button
            ...
            className="w-10 h-10 rounded-full hover:bg-surface-high flex items-center justify-center"
            title="Report"
          >
            <Icon name="flag" />
          </button>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto py-4 space-y-3">
          {messages.map((m) => (
            ...
            >
              ...
              <div
                className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${m.from === "me" ? "bg-primary text-navy rounded-br-sm" : "bg-white border border-border text-navy rounded-bl-sm"}`}
              >
                {m.text}
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {ICEBREAKERS.map((q) => (
                <button
                  key={q}
                  onClick={() => send(q)}
                  className="text-left px-3 py-2 bg-primary-soft hover:bg-primary text-navy rounded-xl text-sm font-semibold transition"
                >
                  {q}
                </button>
              ))}
        ...
        >
          ...
          />
          <FFButton type="submit" disabled={!input.trim()}>
            <Icon name="send" />
          </FFButton>
        </form>
      </div>
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">
              We'll review it within 24h. You'll also be unmatched.
            </p>
            <div className="flex gap-2">
              <FFButton variant="ghost" full onClick={() => setShowReport(false)}>
                Cancel
              </FFButton>
              <FFButton
                variant="coral"
